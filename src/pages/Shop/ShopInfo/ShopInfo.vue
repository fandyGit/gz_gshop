      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item"
               :class="supportsClass[supports.type]"
               v-for="(supports,index) in info.supports"
          >
            <span class="content-tag">
              <span class="mini-tag">{{supports.name}}</span>
            </span>
            <span class="activity-content">{{supports.content}}</span>
          </div>
        </div>
      </section>
...
<script>
  ...
  export default {
    ...
    data(){
      return{
        supportsClass:['activity-green','activity-orange','activity-red'],
      }
    },
    computed:{
      ...mapState(['info'])
    ...
    methods:{
      _initScroll(){
        ...
        const liWidth=120;
        const space=6;
        const lis=this.info.pics.length;
        picsUl.style.width=(liWidth+space)*lis-space+'px';

        this.shopInfo= new BScroll('.shop-info',{
          click:true,
        })
        this.picWrapper= new BScroll('.pic-wrapper',{
          click:true,
          scrollX:true,
        })
      }
